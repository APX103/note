...
3. **更新该分类的 `index.html`**：在合适的 section 里加一张卡片，格式固定：
   ```html
   <a class="card" href="文件名.html">
       <div class="card-title"><span class="arrow">&rarr;</span> 标题</div>
       <div class="card-desc"><span class="tag">标签</span> 一句话描述</div>
   </a>
   ```
   同步更新该 section 标题里的 `(N)` 计数。
4. **视情况更新根 `index.html`** 的「最新记录」区（新文章/重要更新时），并把根页「分类导览」各卡片的篇数计数 +1。
5. 检查文中若有指向库内其他文章的相对链接，路径要带对目录层级。
