...
# Unicode 上下标 → 普通字符映射（聚合连续上下标后交给 LaTeX 命令渲染）
# 西文字体（Times New Roman 等）普遍缺失 U+207x/U+208x 字形，
# 不转换会导致 PDF 里这些字符丢失或显示为方框。
...
def main():
    ...
    # 推断封面占位符（从书名提取主标题/后缀）
    # 例如 "小型模块化反应堆（SMR）技术" → main="小型模块化反应堆", suffix="（SMR）技术", symbol="SMR"
    ...
    if m:
        ...
        # 无英文缩写时，badge/symbol 取书名前 2/4 字（避免截断成半个词的视觉）
