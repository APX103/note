#!/usr/bin/env python3
"""
build.py —— 本书本地构建脚本（基于 STM32G4-Motor-Control-book 的 build.py 定制，
后者又源自 research-to-book skill 的 build.py）

定制点：
1. 模板默认取本书 assets/ 目录（RTB_ASSETS 环境变量仍可覆盖）；
2. escape_text 保留 LaTeX 数学直通：$$...$$ 显示公式与 $...$ 行内公式在转义前
   先被摘出，转义后原样放回，配合 Pandoc 的 tex_math_dollars 扩展输出真公式
   （SDR 为公式密集题材，第 2 章起大量使用）；
3. 作者/封面文案默认值改为本书译本信息。

用法：
    cd SDR-for-Engineers-book
...
def main():
    ...
    ap.add_argument('--book-author', default="李佳伦 译")
    ...
    template_vars = {
        ...
        'COVER_TOPTAG': f"{cover_symbol} · 中译本",
        'COVER_SYMBOL': cover_symbol,
        'COVER_TITLE_MAIN': cover_main,
        'COVER_TITLE_SUFFIX': cover_suffix,
        'COVER_SUBTITLE': book_subtitle or '软件定义无线电 · 从算法到硬件',
        'COVER_AUTHOR': book_author,
        'COVER_SOURCE_NOTE': '译自 Software-Defined Radio for Engineers (Artech House 2018, ADI 官网免费分发) · 仅供个人学习',
