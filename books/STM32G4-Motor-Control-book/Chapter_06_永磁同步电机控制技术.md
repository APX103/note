...
### 6.2.5　ST-MC-SDK Motor Profiler 测量电机参数的原理
...
教科书流程（判扇区 → 套 $T_1/T_2$ 公式 → 七段排布）分支多；嵌入式实现的标准姿势是数学等价的**最小最大值注入法（min-max injection）**——无分支、四次乘法完事：
...
4. 每个 PWM 周期采样两相电流 → Clarke → Park 得到 $i_d$、$i_q$ 反馈；
...
$f_{bw}$ 取 PWM 频率的 1/10 ~ 1/20（20 kHz PWM → 电流环带宽 1 ~ 2 kHz 量级）。$R_s$、$L_s$ 就来自 6.2.4/6.2.5 的测量——**参数辨识 → PI 整定 → 性能**，链条在这里闭合。
...
1. **带宽分离**：速度环带宽取电流环的 1/5 ~ 1/10，内外环才不打架（内环对外环近似" instantaneous"）；
...
- 坐标变换把"三相交流"变成"两个直流"：Clarke（abc→αβ）降维，Park（αβ→dq）换到转子视角；FOC 目标是 $i_d = 0$、$i_q$ 出力；
...
- 电流环带宽法整定 $K_P = 2\pi f_{bw} L_s$、$K_I = 2\pi f_{bw} R_s$，速度环带宽低于电流环 5 ~ 10 倍；
