...
各工具之间的数据流如下：MC Workbench 从电机参数（由 Motor Profiler 测得或手工输入）和板卡信息出发生成完整工程 $\rightarrow$ 在 IDE 里编译烧录 $\rightarrow$ 电机运行时用 MC Monitor / STM Studio 观察、调参。
...
CubeMX 是免费的 Java 桌面应用：在 st.com 注册账号 $\rightarrow$ 搜索 STM32CubeMX $\rightarrow$ 下载对应操作系统的安装包（Win 下为 exe 安装器，macOS 为 dmg）。它也内嵌在 CubeIDE 里（IDE 内即叫 "CubeMX 透视页"）。首次启动会提示下载各系列的固件包，勾选 STM32G4 系列即可。
...
- **System Core $\rightarrow$ Clock Configuration**：时钟树图形化配置，从 HSI/PLL 一路到内核与外设频率，非法配置会红字提示；
- **System Core $\rightarrow$ SYS / GPIO**：调试接口、中断优先级（NVIC）配置；
- **GENERATE CODE**：按所选 IDE 生成工程，用户代码写在生成的注释对（如 `/* USER CODE BEGIN 4 */`）之间，重新生成时保留。

第 4 章的基础实验全部走 "CubeIDE 内建 CubeMX 配置 $\rightarrow$ 生成 $\rightarrow$ 补用户代码” 的流程。
...
**新建一个点灯工程的完整流程**（第 4.1 节的预演）：File $\rightarrow$ New STM32 Project $\rightarrow$ 选板卡 NUCLEO-G431RB（TAB "Board Selector"）$\rightarrow$ 工程名 $\rightarrow$ 在 CubeMX 页把 PA5 设为 GPIO_Output $\rightarrow$ 生成代码 $\rightarrow$ 在 `main.c` 的 while 循环里写翻转语句 $\rightarrow$ 点 Run 烧录。全程约五分钟。
...
MDK 的典型流程：MC Workbench 生成工程时选择 MDK-ARM V5 $\rightarrow$ 得到 `.uvprojx` 工程 $\rightarrow$ 打开后 Configure $\rightarrow$ Debug 里选 ST-Link $\rightarrow$ Flash $\rightarrow$ Download 烧录。MDK 的仿真器（Logic Analyzer 观察全局变量）在做电流环行为分析时很好用。本书截图与步骤以 CubeIDE 为主，MDK 用户在生成工程时选择对应目标即可，源码完全一致。
...
同样由 MC Workbench 生成 IAR 工程（.ewp），打开 $\rightarrow$ Project $\rightarrow$ Download and Debug。IAR 的静态分析与代码体积报告在工程化时用得上，学习阶段与 CubeIDE 无本质差别。

## MotorControl Workbench（MC SDK）
...
6. **生成工程**：选目标 IDE（CubeIDE / MDK / IAR / GCC Makefile）$\rightarrow$ Generate。

生成物是一个完整可编译的工程：`main()` 里初始化电机控制栈，包含应用 API 头文件 `mc_api.h`。第 7 章第一节会把这个流程完整走一遍。

### 使用 ST Motor Profiler 获得电机参数
...
典型用法：新建工程 $\rightarrow$ Import 固件的 axf/elf $\rightarrow$ 勾选要观察的全局变量（如速度、iq 电流）$\rightarrow$ 以 1 kHz 级采样率画曲线 $\rightarrow$ 保存配置。电机调试中"看波形定问题"是核心手段：启动失败看 iq 是否爬升、速度抖动看观测器收敛性、过流看相电流尖峰。
