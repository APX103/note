...
def main():
    ...
    if os.path.exists(mjf):
        ...
        R["MJ_SECTION"] = (
            ...
            "<p>按评审意见重构几何：<b>超薄圆片底盘（r=15cm，Ø30cm）</b>替代半球底座，中央立柱居中接下连杆，"
            "上连杆接<b>圆台（截头圆锥）灯罩</b>侧面，罩口藏摄像头与暖光灯——总质量维持 1.29kg，"
            "扭矩执行器/弹簧/摩擦参数与二维模型一致，另增<b>底盘偏航自由度</b>（Z 轴位置伺服）。"
            "在 MuJoCo 3D 中用同一套结构化策略 + CEM（约 2 分钟/55 代，单集 0.09s，比自研二维仿真快 20 倍）训练向前跳：</p>"
            + f"<table><tr><th>指标</th><th>值</th></tr>"
            ...
            f"<tr><td>转向跳</td><td class='num'>偏航伺服转 90.0° 后朝新方向跳出 ~35cm ✅</td></tr></table>"
